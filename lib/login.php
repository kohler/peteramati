...
class LoginHelper {
    ...
    static private function first_user($user, $msg) {
        global $Conf, $Opt;
        // check for existing chair users
        $result = Dbl::qe("select contactId from ContactInfo where (roles&" . (Contact::ROLE_CHAIR | Contact::ROLE_ADMIN) . ")!=0 limit 1");
        $has_chair = $result && $result->fetch_row();
        if (!$has_chair) {
